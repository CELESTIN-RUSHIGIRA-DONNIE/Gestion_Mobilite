...
if (isset($_POST["add_agents"])) {
    ...
    $sql_insert = "INSERT INTO departement (id_faculte,nom,designation,id_user) 
    VALUES (?,?,?,?)";
    if ($stmt = $con->prepare($sql_insert)) {
        $stmt->bind_param("issi", $faculte_id,$departement,
        $designation, $user_id);
        if ($stmt->execute()) {
            $_SESSION['message'] = "Departement ajouté avec succès.";
            $stmt->close();
            header("Location: departement.php");
            exit;
        } else {
            $_SESSION['message'] = "Erreur lors de l'ajout du departement.";
            header("Location: departement.php");
            exit;
        }
    }
